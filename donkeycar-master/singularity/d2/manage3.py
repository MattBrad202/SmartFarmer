...
from photoPart2 import TakePhotoAndUploadToS3
from bluetoothPart2 import Bluetooth
from shutdownPart import Shutdown
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    V.add(ctr, 
          ...
          outputs=['user/angle', 'user/throttle', 'user/mode', 'recording', 'record', 'bluetooth', 'shutdown'],
          threaded=True)
    
    # ADDED NEW PART, NEED TO TEST
    S3 = TakePhotoAndUploadToS3()
    V.add(S3, inputs=['cam/image_array', 'record'], threaded=True)
    
    bluetooth = Bluetooth()
    V.add(bluetooth, inputs=['bluetooth'], threaded=True)
    
    shutdown = Shutdown()
    V.add(shutdown, inputs=['shutdown'])
    ...
    #run the vehicle at a lower rate for testing purposes
    #V.start(rate_hz=10, max_loop_count=cfg.MAX_LOOPS)
